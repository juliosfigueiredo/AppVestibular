...
class AlamofireAdapterTests: XCTestCase {
    func test_post_shoud_make_request_valid_url_and_method() {
        let url = makeUrl()
    ...
    func test_post_shoud_make_request_with_no_data() {
        testRequestFor(data: nil) { (request) in
            XCTAssertNil(request.httpBodyStream)
        }
    }
    
    func test_post_shoud_complete_with_error_when_request_completes_with_error() {
        expectResult(.failure(.noConnectivity), when: (data: nil, response: nil, error: makeError()))
    }

    func test_post_shoud_complete_with_error_on_all_invalid_cases() {
    ...
    func test_post_shoud_complete_with_data_when_request_completes_with_200() {
        expectResult(.success(makeValidData()), when: (data: makeValidData(), response: makeHttpResponse(), error: nil))
    }
    
    func test_post_shoud_complete_with_data_when_request_completes_with_204() {
    ...
    func test_post_shoud_complete_with_error_when_request_completes_with_non_200() {
...
extension AlamofireAdapterTests {
    ...
    func expectResult(_ expectedResult: Result<Data?, HttpError>, when stub: (data: Data?, response: HTTPURLResponse?, error: Error?), file: StaticString = #file, line: UInt = #line) {
        ...
        sut.post(to: makeUrl(), with: makeValidData()) { receivedResult in
            ...
            default: XCTFail("Excepted \(expectedResult) got \(receivedResult) instead", file: file, line: line)
            }
            exp.fulfill()
        }
        wait(for: [exp], timeout: 1)
